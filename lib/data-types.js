...
class ABSTRACT {
  ...
  /**
   * The dedupe cache above is process-wide, so a test that asserts a warning
   * fires has to clear it first -- the message may already have been emitted by
   * an earlier test file in the same run.
   */
  static clearWarnings() {
    for (const text of Object.keys(warnings)) {
      delete warnings[text];
    }
  }
