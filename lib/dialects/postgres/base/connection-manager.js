...
import { poolAttributes } from '../../../telemetry.js';
import { InstrumentedPool } from './instrumented-pool.js';
...
export class ConnectionManager {
  ...
  /**
   * Stop reporting metrics for every pool this connection manager owns.
   *
   * @private
   */
  _detachPoolMetrics() {
  ...
  initPools() {
    ...
    // Any pool from an earlier call is abandoned here, so stop reporting for it: an observable
    // callback would otherwise keep it alive and keep publishing a pool nothing can acquire from.
    this._detachPoolMetrics();
    ...
    if (!config.replication) {
      this.pool = new InstrumentedPool(
        ...
        poolAttributes(config)
      );

      this.pools.push(this.pool);
    ...
    this.pool = {
      ...
      read: new InstrumentedPool(
        ...
        // The read pool round robins over every replica, so there is no one server to name it
        // after; the first replica stands in for the group.
        poolAttributes(config.replication.read[0], 'read')
      ),
      write: new InstrumentedPool(
        {
          create: async () => {
            try {
        ...
        poolAttributes(config.replication.write, 'write')
      )
    };

    this.pools.push(this.pool.read, this.pool.write);
