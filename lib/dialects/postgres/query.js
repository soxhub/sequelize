...
import { UPSERT_CREATED_ALIAS } from './query-generator.js';
...
export class Query extends BaseQuery {
  ...
  async run(sql, parameters) {
    ...
    if (this.isShowIndexesQuery()) {
      ...
      // `ON CONFLICT ... DO UPDATE` always returns exactly the one row it touched, alongside the
      // synthetic created flag. The flag shares the row with the model's own columns, so it has to
      // come off before the rest are mapped onto the instance.
      const row = rows[0] || {};
      const created = row[UPSERT_CREATED_ALIAS];

      if (this.instance && this.instance.dataValues) {
        for (const key in row) {
          if (key === UPSERT_CREATED_ALIAS || !Object.hasOwn(row, key)) {
            continue;
          }
          ...
          this.instance.dataValues[(attr && attr.fieldName) || key] = row[key];
        }
      }
  ...
  formatError(err) {
    ...
    switch (code) {
      ...
      // A 42704 that is not about a constraint is a plain database error:
      // falls through
      default:
