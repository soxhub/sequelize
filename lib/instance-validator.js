...
export class InstanceValidator {
  ...
  _builtinValidators() {
    const validators = [];
    for (const [field, rawAttribute] of Object.entries(this.modelInstance.rawAttributes)) {
      ...
      if (Object.hasOwn(this.modelInstance.validators, field)) {
        validators.push(this._builtinAttrValidate(value, field));
      }
    }
  ...
  async _builtinAttrValidate(value, field) {
    ...
    for (const [validatorType, validatorSpec] of Object.entries(fieldValidators)) {
      let test = validatorSpec;
  ...
  async _invokeCustomValidator(validatorFn, validatorType, optAttrDefined, optValue, optField) {
    ...
    try {
      ...
      return this._pushError(false, errorKey, err, optValue, validatorType);
    }
  }
