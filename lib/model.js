...
class Model {
  ...
  static async findOrCreate(options, ...rest) {
    ...
    try {
      ...
      // `ON CONFLICT DO NOTHING` rather than an INSERT wrapped in a plpgsql exception handler: a
      // conflict stops being an error, so the statement writes no pg_temp function, opens no
      // subtransaction, and leaves the surrounding transaction usable whichever way it goes.
      options.ignoreDuplicates = true;
      options.returning = true;

      const created = await this.create(values, options);

      if (!created._skippedDuplicate) {
        return [created, true];
      }

      // Some unique index rejected the row. Which one, and over what value, is what the rest of this
      // method needs, and only Postgres can say -- so replay the insert through the exception
      // wrapper to collect it. That costs a second INSERT, but only on a path that was already
      // heading for either a concurrent-insert recovery or a throw; the common case returned above
      // without ever writing a pg_temp function or opening a subtransaction.
      //
      // Replaying re-runs `beforeCreate`. That is safe for the hooks this path exists to serve --
      // they normalise values -- but a hook with side effects would fire a second time here.
      delete options.ignoreDuplicates;
   ...
   * @param  {Boolean|Array} [options.returning=false] Return the upserted row alongside the created flag, changing the resolved value to `<Model, created>`. The row comes back from the statement's own RETURNING clause and populates the instance in place, so this costs no extra query. Pass an array of attribute names to narrow the RETURNING clause to those columns; attributes outside the list keep the values they were built with rather than the ones the database now holds
   * @param  {Array}        [options.conflictFields] Columns to use as the `ON CONFLICT` target, bypassing arbiter resolution
   * @param  {Object}       [options.conflictWhere] Predicate identifying a partial unique index to use as the `ON CONFLICT` target
   * @param  {Transaction}  [options.transaction] Transaction to run query under
   * @param  {Function}     [options.logging=false] A function that gets executed while running the query to log the sql.
   * @param  {Boolean}      [options.benchmark=false] Pass query execution time in milliseconds as second argument to logging function (options.logging).
   * @param  {String}       [options.searchPath=DEFAULT] An optional parameter to specify the schema search_path (Postgres only)
  ...
  static async upsert(values, options) {
    ...
    // Callers name conflict fields the way they name attributes, but the conflict target is spliced
    // into SQL verbatim, so it has to be the column name.
    if (options.conflictFields) {
      options.conflictFields = options.conflictFields.map(
        (attrName) => (this.rawAttributes[attrName] && this.rawAttributes[attrName].field) || attrName
      );
    }
    ...
    // Attach createdAt. The guard reads `insertValues`, not `updateValues`: createdAt is deliberately
    // left out of the update half, so testing that half means the caller's own explicit createdAt was
    // overwritten by `now` whenever it fell outside `options.fields`.
    if (createdAtAttr) {
      const field = this.rawAttributes[createdAtAttr].field || createdAtAttr;
      if (!insertValues[field]) {
    ...
    // The RETURNING clause populates this instance in place, so `returning: true` costs no extra
    // round trip.
    options.instance = instance;

    const [record, created] = await this.QueryInterface.upsert(
      this.getTableName(options),
      insertValues,
      updateValues,
      this,
    ...
    let result;
    // Any truthy `returning`, not just `true`: a column list narrows the RETURNING clause, and the
    // row it does select is still worth handing back. Attributes outside the list keep the values
    // they were built with rather than the ones the database now holds.
    if (options.returning && record) {
  ...
  static async bulkCreate(records, options) {
    ...
    if (options.individualHooks) {
      ...
      for (const attr of Object.keys(this.tableAttributes)) {
    ...
    instances.forEach((instance) => {
      for (const [attr, rawAttribute] of Object.entries(this.rawAttributes)) {
        if (
          rawAttribute.field &&
          instance.dataValues[rawAttribute.field] !== undefined &&
          rawAttribute.field !== attr
        ) {
          instance.set(attr, instance.dataValues[rawAttribute.field]);
          delete instance.dataValues[rawAttribute.field];
        }
  ...
  */
  ...
  async save(options, ...rest) {
    ...
    // `ignoreDuplicates` turns a unique violation into a silently skipped row, so the statement
    // succeeds either way and nothing else here can tell the two apart. Record which happened:
    // reading it back off the primary key only works when the database assigned one, which is not
    // the case for a caller-supplied key. Non-enumerable so it stays out of `dataValues`, JSON and
    // every other walk of the instance.
    if (options.ignoreDuplicates) {
      Object.defineProperty(this, '_skippedDuplicate', {
        value: rowsUpdated === 0,
        configurable: true,
        enumerable: false,
        writable: true
   ...
   */
  // oxlint-disable-next-line no-unused-vars -- documentation-only stub; the real method is installed by the association mixin
