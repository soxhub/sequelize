...
export class QueryInterface {
   ...
   * Upsert
   *
   * Resolves the single arbiter index that `ON CONFLICT` will key on, then hands off to
   * `upsertQuery`. The resolution order matches Sequelize 6/7 so that models with both a v4
   * definition here and a migrated counterpart in the Sequelize 6 package agree on which
   * constraint an upsert conflicts against: an explicit `conflictFields`, else the first unique
   * key or unique index covering one of the updated columns, else the primary key. The primary
   * key also wins outright whenever it is among the columns being updated.
   *
   * @param {String} tableName
   ...
   * @param {Model}  model
   * @param {Object} options
   * @param {Array}  [options.conflictFields] columns to use as the conflict target, bypassing resolution
   * @param {Object} [options.conflictWhere]  predicate selecting a partial unique index as the arbiter
   *
   * @returns {Promise<[Model, Boolean]>} the upserted instance and whether it was inserted
   */
  async upsert(tableName, insertValues, updateValues, model, options) {
    options = _.clone(options);
    options.type = QueryTypes.UPSERT;
    options.raw = true;
    options.model = model;

    const updateOnDuplicate = Object.keys(updateValues);
    options.upsertKeys = options.conflictFields || [];

    if (options.upsertKeys.length === 0) {
      const primaryKeys = Object.values(model.primaryKeys).map((item) => item.field);

      const uniqueKeys = Object.values(model.options.uniqueKeys)
        .filter((c) => c.fields.length > 0)
        .map((c) => c.fields);

      const indexKeys = model.options.indexes
        .filter((c) => c.unique && c.fields.length > 0)
        // fields in the index may be either strings or objects with an attribute property
        .map((c) => c.fields.map((field) => (_.isPlainObject(field) ? field.attribute : field)));

      for (const field of updateOnDuplicate) {
      ...
      if (options.upsertKeys.length === 0 || _.intersection(updateOnDuplicate, primaryKeys).length > 0) {
    ...
    const sql = this.QueryGenerator.upsertQuery(tableName, insertValues, updateValues, model, options);
